...
/*
 * Delays the response for a json request until the amount of milliseconds in delay passed
 */
void jsonRpc_delayRequestByTime(jsonRpcServer_t* jrs, jsonRpcClient_t* client, jsonObject_t* jsonObject, uint32 delay, void (*jsonRpc_processRequest_handler)(jsonRpcServer_t* jrs, jsonRpcClient_t* client, bool isDelayed))
{
	client->hasTimedDelay = true;
	client->timerDelay = GetTickCount() + delay;
	client->delayedRequestData.jsonObject = jsonObject;
	client->delayedRequestData.jsonRpc_processRequest_handler = jsonRpc_processRequest_handler;
}

/*
 * Delays the response for a json request until the condition object is alerted
 */
void jsonRpc_delayRequest(jsonRpcServer_t* jrs, jsonRpcClient_t* client, jsonObject_t* jsonObject, jsonRpcDelayCondition_t* condition, void (*jsonRpc_processRequest_handler)(jsonRpcServer_t* jrs, jsonRpcClient_t* client, bool isDelayed))
{
	// todo: condition handling & wakeup
}

/*
 * Will wake up all waiting (delayed) requests that have set the same parameters in their notification
 */
void jsonRpc_conditionNotification(jsonRpcServer_t* jrs, uint32 reason, uint32 id1, uint32 id2)
{
	// todo
}
...
void jsonRpc_processRequest(jsonRpcServer_t* jrs, jsonRpcClient_t* client)
{
	...
	if( client->lastRequestJsonData != NULL )
		jsonObject_freeObject(client->lastRequestJsonData);
	jsonObject_t* jsonObject = jsonParser_parse((uint8*)requestData, requestLength);
	client->lastRequestJsonData = jsonObject;
	// get method
	...
	client->lastRequestJsonParameter = jsonParameter;
	...
	if( methodNameString )
	{
		...
		{
			jsonRpc_processRequest_getwork(jrs, client, NULL);
		}
		else
		{
...
int jsonRpc_run(jsonRpcServer_t* jrs)
{
	...
	while( true )
	{
		...
		if( r )
		{
			...
			Sleep(1);
		// check if we have to trigger delayed requests
