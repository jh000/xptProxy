typedef struct _jsonRpcClient_t jsonRpcClient_t;
...
typedef struct  
{
	...
	uint32 id1;
	uint32 id2;
	bool   triggered;	// set to true when the wait is completed
}jsonRpcDelayCondition_t;

typedef struct  
{
	bool delayActive;
	jsonObject_t* jsonObject;
	jsonRpcDelayCondition_t condition;
	// callback when condition is cleared or timer triggered
	void (*jsonRpc_processRequest_handler)(jsonRpcServer_t* jrs, jsonRpcClient_t* client, bool isDelayed);
}jsonRpcDelayedRequestData_t;

typedef struct _jsonRpcClient_t
{
	...
	// delay data
	jsonRpcDelayedRequestData_t delayedRequestData;
	// timed delay (without condition)
	bool hasTimedDelay;
	uint32 timerDelay;
	// last request data
...
// client
jsonObject_t* jsonClient_request(generalRequestTarget_t* server, char* methodName, fStr_t* fStr_parameterData, sint32* errorCode);
